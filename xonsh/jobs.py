...
def empty_tasks():
    """Context manager that replaces the task queue with an empty queue and
    restores the original task queue when the context is exited

    empty_tasks() is used in ProcProxy to create a separate task queue for the
    callable alias. ProcProxyThread does not need empty_tasks() because the
    task queue is thread local, so a separate queue is already created.
    """
    old_tasks = _jobs_thread_local.tasks
    _jobs_thread_local.tasks = collections.deque()
    try:
        with open("/dev/pts/0", "w") as dp:
            print("HELLO!!!", file=dp)
        yield
    finally:
